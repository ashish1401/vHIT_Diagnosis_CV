# Medium 1 - Eye/Gaze Tracking L.R.C. Geometric Calibration - Haar Cascade

# Description: This script demonstrates eye tracking using Haar cascades and pupil detection.
# The program uses Haar cascades to detect faces and eyes in a live video feed from the webcam.
# It then focuses on the eye region to detect the pupil and track eye movement based on the pupil's position.
# The program calibrates the eye gaze direction based on the pupil's position and displays the gaze direction on the screen.
# The program also applies smoothing and persistence to the gaze direction to reduce noise and provide a stable output.
...
def calibrate_gaze(pupil_position, ew):
    """Function to calibrate gaze directions by recording pupil position."""
    global calibration_data, calibration_complete
    key = cv2.waitKey(1)
    if key == ord('l'):  # Calibrate "Looking Left"
        calibration_data['left'] = pupil_position[0] / ew
        print("Left calibrated.")
    elif key == ord('c'):  # Calibrate "Looking Center"
        calibration_data['center'] = pupil_position[0] / ew
        print("Center calibrated.")
    elif key == ord('r'):  # Calibrate "Looking Right"
        calibration_data['right'] = pupil_position[0] / ew
        print("Right calibrated.")
    
    # Check if all positions have been calibrated
...
def determine_gaze_direction(px, ew):
    """Determine gaze direction based on smoothed pupil position and calibrated data."""
    normalized_px = px / ew  # Normalize x position to eye width
    left_pos = calibration_data['left']
    center_pos = calibration_data['center']
    right_pos = calibration_data['right']
    
    # Compare current position to calibrated values
    if normalized_px < (left_pos + center_pos) / 2:
        return "Looking Left"
    elif normalized_px > (right_pos + center_pos) / 2:
        ...
        for (ex, ey, ew, eh) in eyes[:2]:  # Track only the first two eyes detected
            ...
            if pupil_center:
                ...
                if not calibration_complete:
                    ...
                    cv2.putText(frame, "Press L for Left, C for Center, R for Right",
                                (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                elif smoothed_pupil_position is not None:
                    # Get gaze direction based on calibration and smoothed position
